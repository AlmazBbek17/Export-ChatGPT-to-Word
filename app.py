...
def _code(doc, code, lang=''):
    p = doc.add_paragraph()
    if lang:
        lang_run = p.add_run(lang + '\n')
...
def health():
    return jsonify({'status': 'OK', 'version': '1.0-chatgpt'})
...
def export_chat():
    try:
        ...
        title = data.get('title', 'ChatGPT Chat')
        ...
        for i, msg in enumerate(messages):
            ...
            rr = rp.add_run('You' if role == 'user' else 'ChatGPT')
            rr.bold = True
            rr.font.size = Pt(14)
            # User: blue, Assistant: green
            rr.font.color.rgb = RGBColor(33, 150, 243) if role == 'user' else RGBColor(16, 163, 127)
        ...
        return send_file(
            ...
            as_attachment=True,
            download_name='chatgpt-chat.docx'
        )
